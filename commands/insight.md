...
- **插件根路径**:代码块里一律用 `${CLAUDE_PLUGIN_ROOT}` 指插件安装根(CC 注入 prompt 前替换成绝对路径,双引号包路径)。**别 `cd` 进插件根**——会破坏 `--project "$(basename "$PWD")"` 取当前 session cwd basename 的语义(那要的是本 session 的项目名,不是插件根名)。
...
1. **先试 Mode A**(本插件 hook 落的 live JSONL,完整深度):
   ```bash
   python3 "${CLAUDE_PLUGIN_ROOT}/tools/analyze.py" --project "$(basename "$PWD")" --json
   ```
   (**不 cd** — 保持 `cwd` = 当前 session 项目目录,`basename "$PWD"` 才是本 session 的 live project 名。)
2. **若 `recordsTotal=0`**(hook 没注册 / 当前 session 没落盘)→ 降级 **Mode B**(读当前 CC session transcript,depth-2,平台边界):
   ...
   python3 "${CLAUDE_PLUGIN_ROOT}/tools/analyze.py" --transcript "$SID" --json
   ```
...
4. **给 dashboard URL**:`http://127.0.0.1:${AGENTINSIGHT_PORT:-8765}`。若 server 未跑,提示 `python3 "${CLAUDE_PLUGIN_ROOT}/dashboard/server.py"` 起(或问用户要不要现在起)。

---

## `live` —— 切 live 源 tail

1. 确保 dashboard server 在跑;未跑则用绝对路径启:`python3 "${CLAUDE_PLUGIN_ROOT}/dashboard/server.py" --source live`。
...
1. `python3 "${CLAUDE_PLUGIN_ROOT}/tools/analyze.py" --scan-projects --json`(扫全 `${AGENTINSIGHT_PROJECTS_ROOT:-~/.claude/projects}`)。
...
1. 定位 transcript(find 绝对路径 + `--`):
   ...
   ```
2. `python3 "${CLAUDE_PLUGIN_ROOT}/tools/analyze.py" --transcript <path> --tree --json`。
