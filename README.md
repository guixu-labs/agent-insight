...
| env | 作用 | 默认 |
|---|---|---|
| `AGENTINSIGHT_LOG_DIR` | JSONL 根目录 | `CLAUDE_PLUGIN_DATA` → `~/.claude/agent-insight` |
...
**2. pip from git(CLI 用户)** —— `pip install git+https://github.com/guixu-labs/agent-insight`。拿到 `agent-insight` CLI(dashboard / analyze 入口)。不从 PyPI 取,故包名无需在 PyPI 占位。

**3. 零依赖 clone(stdlib only)** —— `git clone` 后直接 `python3 dashboard/server.py` 或 `python3 tools/analyze.py`。纯标准库,无 pip install。
