...
| 跨 session 续接(SessionStart hook + lineage 缝合 + 预算计量) | ✅ 已交付(lineage 缝合建满;budgetState 跨 session 计量已交付,reader-computes,opt-in) |
...
| `AGENTINSIGHT_BUDGET_THRESHOLD` | 按 generationId 跨 session 卷起累计预算的 token 阈值(reader 离线 budgetState);**也是实时 emission 总开关**(配了 recorder 才实时算+发) | 无 → 不显 budget chip、不发 emission |
| `AGENTINSIGHT_BUDGET_WEBHOOK` | 配了则每次 Agent 落盘后实时 POST `BudgetEvent` 给该 URL(外部 handoff 工具订阅);**前提 `BUDGET_THRESHOLD` 也配了** | 无 → 只写本地 `budget-events.jsonl` |

### 预算计量(budgetState)

设 `AGENTINSIGHT_BUDGET_THRESHOLD` 为一个 token 数(如 `60000`),reader 按 `generationId` 把同一逻辑工作流的多个 session 的 `grandTotal.total` **跨 session 卷起求和**(cacheRead-inclusive,与计费核同口径),挂出 `result.generations[i].budgetState = {threshold, cumulativeTotal, pctOfThreshold, exceeded}`。dashboard 在 fleet 表 total 列 + 续接组头显 budget chip:<80% 绿 / 80–99% 琥珀 / ≥100%(到阈即超)红。**未设阈值 → 字段不存在 → 表格与今天逐字一致**(graceful opt-in,不新增永久列)。判定逻辑单一源头(`tools/budget.py`,reader 离线 + recorder 实时 emission 共用)。

**实时预算事件 emission(per-session,区别于上面的跨 session 离线)**:配了 `AGENTINSIGHT_BUDGET_THRESHOLD` 后,每次 Agent 落盘 recorder 还会**实时**算**本 session**累计(对应外部 handoff 动作层的 per-session handoff 语义:单 session 跑爆 → 立即触发),写 `<log_base>/budget-events.jsonl`;配了 `AGENTINSIGHT_BUDGET_WEBHOOK` 则额外 POST。给外部 handoff 工具实时订阅 —— 与上面跨 session budgetState(离线回顾)是两回事。opt-in(阈值没配则不发,守零耦合)。
...
**🔴 平台硬边界**:CC transcript **只对 root 直发 Agent 调用持久化结构化 `toolUseResult`**;嵌套调用(子 agent 再派子 agent)只落成 message content 文本块、该行 `toolUseResult=null` → **Mode B 只重建 depth-2(root→agent),depth-3+ token / 拓扑须靠 live hook**。三独立真实样本验证全 depth-2、零嵌套。**2026-06-24 live 补测**:`general-purpose` subagent 默认不递归派 Agent(CC 编排基本卡在 depth-2);depth-3+ 归因能力保留(reader 合成单测验过),但 live 真数据以 depth-2 为主 —— 真正稳的卖点是 **depth-2 主流的精准 per-subagent 归因 + 拓扑 + 单一计费核**。
...
## live 自检(整体 hook→record→reader 验证)

合成单测覆盖不了「真 CC 平台行为」——hook 是否真 fire、token 是否真透传、depth-3+ 嵌套是否真录到。

**前置**:先按上面「安装」装好插件(`/plugin marketplace add guixu-labs/agent-insight` → `/plugin install agent-insight`,或项目级 `.claude/settings.local.json` 手挂 `PostToolUse` hook、`command` 用 `hooks/record.py` 绝对路径),并在**隔离 New Session** 里跑(红线 3/F7:hook 配置 mid-session 不重载,重启才 fire)。hook 没 fire → `live_check` 报 `NO_DATA`。

⚠️ **关键**:`live_check` **只读已落盘的数据,自己不产生数据**——直接无参跑会 `NO_DATA`(这个 session 还没编排活动)。必须**先**用 `--show-probe` 拿 prompt 喂给 CC,让 CC 派 subagent 触发 hook 落盘,**再**跑验证:

```bash
# 第 1 步(产生数据,必须先做):拿触发 prompt → 复制喂给 CC → CC 派 subagent → hook 落盘
python3 tools/live_check.py --show-probe agent      # Agent 轨基本落盘(派 general-purpose 回 pong)
python3 tools/live_check.py --show-probe nested     # (可选)depth-3+ 嵌套,验核心 moat

# 第 2 步(验证):CC 跑完后,读落盘 + 全套断言 + verdict
python3 tools/live_check.py
```

verdict:`LIVE_OK`(核心链路立住:Agent 轨落盘 + token 命门 + 自洽)/ `CORE_FAIL`(命门不过,带结果排查 `hooks/record.py` 字段路径)/ `NO_DATA`(hook 没挂上)。depth-3+ 嵌套捕获(核心 moat)单独报告——录到且嵌套层 caller 正确即 moat 成立,否则诚实提示(LLM 没派满 / 发现平台限制)。状态报告器,绝不阻断编排(`exit 0`,仅真异常 `exit 1`)。详见 `python3 tools/live_check.py --help`。
