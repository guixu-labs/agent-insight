...
| `AGENTINSIGHT_BUDGET_THRESHOLD` | 按 generationId 跨 session 卷起累计预算的 token 阈值(reader-computes,opt-in) | 无 → 不显 budget chip |

### 预算计量(budgetState)

设 `AGENTINSIGHT_BUDGET_THRESHOLD` 为一个 token 数(如 `60000`),reader 按 `generationId` 把同一逻辑工作流的多个 session 的 `grandTotal.total` **跨 session 卷起求和**(cacheRead-inclusive,与计费核同口径),挂出 `result.generations[i].budgetState = {threshold, cumulativeTotal, pctOfThreshold, exceeded}`。dashboard 在 fleet 表 total 列 + 续接组头显 budget chip:<80% 绿 / 80–99% 琥珀 / ≥100%(到阈即超)红。**未设阈值 → 字段不存在 → 表格与今天逐字一致**(graceful opt-in,不新增永久列)。判定逻辑单一源头(reader-computes),未来 Breather 实时事件复用同源(emission 仍 defer)。
